...
1. In ReportPortal's UI, create a project and generate an API key (Profile page).
2. Set in this repo's `.env` (see `.env.example` for the full list): `RP_ENDPOINT`, `RP_API_KEY`,
   `RP_PROJECT`, optionally `RP_LAUNCH`.
3. Run tests as usual (`npm test`, `npm run test:ui`, ...). `playwright.config.ts` only adds the
   ...
   changes and no connection is attempted.
