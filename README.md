...
| 11  | [Smart Retry / Prioritization](#not-yet-built)                |  ⚪ 0%  | Not built — `retry-dispatch.ts` produces a per-failure retry budget, but nothing consumes it: Playwright's `retries` is fixed per project before the run starts, so acting on it needs a runner-level change, not just a caller | Would let CI stop wasting retries on failures that can never pass on retry (e.g. a missing env var) and spend them only where retrying can actually help                                                                                                                            |
