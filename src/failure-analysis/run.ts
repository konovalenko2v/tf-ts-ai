...
const RETRY_VERDICTS: Record<FailureCategory, string> = {
  config:
    'Retrying cannot fix this — same missing value every attempt, budget should be 0. A globalSetup precheck (src/core/global-setup.ts) already guards BOOKER_USERNAME/USER_PASSWORD before any retries are spent; other config failures still burn the full retry budget until they get the same guard.',
  'ai-quota':
